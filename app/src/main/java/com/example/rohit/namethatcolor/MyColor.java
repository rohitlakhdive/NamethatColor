...
class MyColor {
    ...
    private int red, blue, green, hue, saturation, value;
    ...
    MyColor(String color) {
        ...
        this.value = (int) hsv[2] * 255;

    }

    static String[] getName(String color) {
        ...
        input = new MyColor(color);

        for (int i = 0; i < colorList.length; i++) {

            check = new MyColor(colorList[i][0]);
            del = delta(input, check);
            if (delLast == -1 || delLast > del) {
                delLast = del;
                resultIndex = i;
            }
        ...
        if (resultIndex == -1) {
            ...
            result[0] = "Invalid Color";
            result[1] = "Invalid Color";

        } else {

            result[0] = colorList[resultIndex][1];
            result[1] = colorList[resultIndex][2];
    ...
    static int delta(MyColor input, MyColor check) {

        int delta1 = (int) (Math.pow(input.red - check.red, 2) + Math.pow(input.green - check.green, 2) + Math.pow(input.blue - check.blue, 2));
        int delta2 = (int) (Math.pow(input.hue - check.hue, 2) + Math.pow(input.saturation - check.saturation, 2) + Math.pow(input.value - check.value, 2));
